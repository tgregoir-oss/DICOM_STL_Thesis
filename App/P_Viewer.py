...
class P_Viewer(Frame):
    def __init__(self, parent):
        ...
        self.DisplayButton = Button(self, text="Display information from a DICOM file", command=self.select_to_print)
        self.DisplayButton.place(x=50, y=80, width=550, height=35)

        self.ExtractButton = Button(self, text="Extract STL from DICOM STL Encapsulated", command=self.select_DICOM)
        self.ExtractButton.place(x=50, y=130, width=550, height=35)

        self.IntracrButton = Button(self, text="Encapsulate STL file into an empty DICOM", command=self.select_STL)
        self.IntracrButton.place(x=50, y=180, width=550, height=35)
    ...
    def select_to_print(self):
        filetypes = [('DICOM File', '*.dcm')]
        file = filedialog.askopenfilename(initialdir=os.getcwd(), title="Choose a file", filetypes=filetypes)
        if file:
            print("File to display :" + os.path.dirname(file))
            print(pydicom.dcmread(file, force=True))
