...
SmoothOption = ['True','Pre-processing','Post-processing', 'None']
...
class P_RT_Struct(Frame):
    ...
    def select_files(self):
        ...
        files = filedialog.askopenfilenames(initialdir=os.getcwd(), title="Sélectionner des fichiers",
                                            filetypes=filetypes)
        ...
        if files:
            for f in files:
                ...
                if (DS.Modality == "RTSTRUCT"):
                    ...
                    Process_Files.process_DICOM_RT_Strcut(f, definition=size,
                                                          save_external_stl=self.ComboOutput.current(),
                                                          Smoothing=self.ComboSmooth.current(),
                                                          fichier=self.ComboSTL.current())
                else:
